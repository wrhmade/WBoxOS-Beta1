...
{
	struct BOOTINFO *binfo = (struct BOOTINFO *) ADR_BOOTINFO;
	putfonts8_asc(binfo->vram, 0, 0, 0, COL8_FFFFFF, "Starting Up...");
	...
	*((int *) 0x0fe8) = (int) nihongo;
	...
	for (;;) {
		...
		//putfonts8_asc(binfo->vram, binfo->scrnx, 615, 523, COL8_000000, "WBoxOS Operating System");
		putfonts8_asc(binfo->vram, binfo->scrnx, binfo->scrnx-185, binfo->scrny-77, COL8_FFFFFF, "WBoxOS Operating System");
		//putfonts8_asc(binfo->vram, binfo->scrnx, 655, 539, COL8_000000, "Version 0.1 [Beta]");
		putfonts8_asc(binfo->vram, binfo->scrnx, binfo->scrnx-145, binfo->scrny-61, COL8_FFFFFF, "Version 0.1 [Beta]");

		//putfonts8_asc(binfo->vram, binfo->scrnx, 385, 555, COL8_000000, "WARNING:THIS IS A BETA VERSION THAT MAY BE UNSTABLE.");
		putfonts8_asc(binfo->vram, binfo->scrnx, binfo->scrnx-415, binfo->scrny-45, COL8_FFFFFF, "WARNING:THIS IS A BETA VERSION THAT MAY BE UNSTABLE.");
		//?示日期
		sprintf(s, "%02d/%02d/%02d %s", get_year()%100, get_mon_hex(), get_day_of_month(),result);
		putfonts8_asc_sht(sht_back, binfo->scrnx - 150, binfo->scrny -20, COL8_FFFFFF, 0, s, 15);
		//?示??
		sprintf(s, "%02d:%02d", get_hour_hex(), get_min_hex());
		putfonts8_asc_sht(sht_back, binfo->scrnx - 45, binfo->scrny -20, COL8_FFFFFF, 0, s, 5);
...
struct SHEET *open_console(struct SHTCTL *shtctl, unsigned int memtotal)
{
	...
	sheet_setbuf(sht, buf, 600, 450, -1); /* 透明色なし */
	make_window8(buf, 600, 450, "WBoxOS Operating System", 0);
